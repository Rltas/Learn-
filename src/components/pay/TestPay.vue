<template>
  <div class="shoppingcar-pay">
    ...
    <div class="maincon" style="padding:.42rem 0;">
        ...
        <div class="section">
          <div class="ord-list">
            ...
            <div class="shop-item" v-for="(shopList,index) in newCarShoppingList">
              ...
              <!-- 发票 -->
              <van-cell title="选择开票类型" :value="shopList.billName" is-link @click="chooseDistribution2(shopList,index)">
              </van-cell>
              <van-cell v-show="shopList.bill_type == 1" title="个人发票还是公司发票" :value="shopList.personOrBusiness" is-link
                @click="chooseBill(index)"></van-cell>
              <van-field v-show="shopList.bill_type == 1 && shopList.bill == 1" placeholder="请输入单位名称"
                v-model="shopList.bill_content" rows="1" maxlength="50" autosize></van-field>
              <van-field v-show="shopList.bill_type == 1&& shopList.bill == 1" label="纳税识别码" placeholder="或统一社会信用代码"
                v-model="shopList.bill_no" rows="1" autosize></van-field>
      ...
      <!-- 备注 -->
      <van-field type="textarea" maxlength="150" label="备注" v-model="remark" placeholder="请填写150字以内备注信息" show-word-limit
        rows="2" style="margin-top:.1rem"></van-field>
      <!-- 结算卡片 -->
...
<script>
  ...
  export default {
    ...
    watch: {
      newCarShoppingList: {
        handler(val) {
          if (this.newCarShoppingList.length == 0) {
            return
          }
          let judge = this.newCarShoppingList.every((item, index) => item.transportPrice >= 0)
          if (!judge) return;
          let transportTotlePrice = 0;
    ...
    methods: {
      loadAddrss() {
        this.api.post("getAddressList", {
          ...
          let _this = this;
          this.newCarShoppingList.map((item, index) => {
              setTimeout(() => {
                this.autoGainFreight(item, index);
              }, index * 500)
          })
      ...
      autoGainFreight(shopList, index) {
        ...
        if (this.addressData > 0) {
          ...
          shopList.goodsInfos.forEach(item => {
            ...
            $.ajax({
              url: global.COURSES + "/app/goods_transport_money.htm",
              data: {
              ...
              this.newCarShoppingList[
                this.shopListIndex
              ].transportDefaultValue = this.transportMoney[0].key;
              this.newCarShoppingList[
                this.shopListIndex
              ].transportName = this.transportMoney[0].key;
              this.newCarShoppingList[
                this.shopListIndex
              ].transportPrice = this.transportMoney[0].value;
